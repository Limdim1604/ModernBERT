...
Với ModernBERT-large, do mô hình lớn hơn và tốn VRAM hơn, batch size được giảm xuống 4 để tránh lỗi CUDA out of memory. Trong một số trường hợp như ANLI, có dùng thêm gradient accumulation để tăng effective batch size mà không vượt quá giới hạn bộ nhớ GPU. 
...
Đối với GLUE, learning rate, weight decay và epochs được đặt theo Table 6 của paper. Đối với ANLI Round 1, vì paper không cung cấp hyperparameter cho dataset này, chúng tôi chọn cấu hình gần với các task NLI trong GLUE và điều chỉnh số epoch theo thực nghiệm.
